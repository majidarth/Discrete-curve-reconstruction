...
def parse_args():
    #parsing arguments for image size, source, type of experiment and implementation
    ...
    parser.add_argument("--diracs", required=False, help="numpy matrix file containing source and sink nodes")
    parser.add_argument("--max_pot", required=False, help="if source and sink nodes are to be chosen at random, they will be chosen where the potential is under max_pot")
    parser.add_argument("--points_close", required=False, help="whether or not random source and sink pairs need to be chosen close to each other", action="store_true")
    parser.add_argument("--n_moves", required=False, help="number of times source and sink nodes need to move")
...
def potential_init(N, M, image):
    ...
    if image != None:  # assembling potential from image
        img = (1 - np.mean(np.array(plt.imread(image)), axis=2))**2 # arbitrary rescaling of the greyscale image        
        ...
        #grid_potential = scp.ndimage.gaussian_filter(grid_potential, sigma=2) #gaussian filter to smoothen potential
...
def edge_potential(N, M, grid_potential):
    #simple averaging to define potential on graph
